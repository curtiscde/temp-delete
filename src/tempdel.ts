...
const getTotalFileSize = (files: IFile[]): number => files.map(f => f.size).reduce((x,y) => x + y, 0);
...
const logFilesToBeDeleted = (filesToBeDeleted: IFile[]) => {
  ...
  if (filesToBeDeleted.length > 100) {
    filesToBeDeleted.forEach(file => {
      console.log(`${file.path} | ${file.modifiedDate!.toDateString()} | ${formatBytesToMB(file.size)}`);
    })
  }
  else {
    const tbody: any[] = [];
    ...
    filesToBeDeleted.forEach(file => {
      if (i < 100) {
        tbody.push([file.path, file.modifiedDate!.toDateString(), formatBytesToMB(file.size)])
        i = i + 1;
      }
    });
...
const main = async () => {
  ...
  const percentageToBeDeleted = ((filesToBeDeletedFileSize / totalFileSize) * 100).toFixed(2);
  ...
  new Table()
    ...
    .body([
      ...
      ['To be deleted', filesToBeDeleted.length, filesToBeDeletedFileSizeMb, percentageToBeDeleted]
    ]).border(true).render();
  ...
  if (confirm(`Do you wish to delete the ${filesToBeDeleted.length} files that are older than ${daysUntilDelete} days?`)) {
